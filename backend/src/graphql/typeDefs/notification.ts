  ...
  definition(t) {
    t.list.field('getNotifications', {
      ...
      resolve: async (_, __, ctx) =>
        await NotificationService.getNotifications(ctx),
    });
  },
});
  ...
  definition(t) {
    t.field('markNotificationAsRead', {
      ...
      resolve: async (_, { id }, ctx) =>
        await NotificationService.markAsRead(id, ctx),
    });
...
export const notificationSubscription = subscriptionField(
  'notificationCreated',
  {
    type: 'Notification',
    subscribe: withFilter(
      (_, __, { pubsub, user }: any) => {
        console.log(
          `[WS] Subscribing. User: ${user?.email || 'anonymous'} (ID: ${user?.userId || 'none'})`
        );
        return pubsub.asyncIterator(EVENTS.NOTIFICATION_CREATED);
      },
      (payload, _, { user }: any) => {
        const recipientId = payload.notificationCreated.recipientId;
        const currentUserId = user?.userId;
        const match = recipientId === currentUserId;
        console.log(
          `[WS] FILTER: recipientId=${recipientId} currentUserId=${currentUserId} MATCH=${match}`
        );
        return match;
      }
    ) as any,
    resolve: (payload: any) => {
      console.log(
        `[WS] Resolving notification: ${payload.notificationCreated.id}`
      );
